...
import (
	...
	"os"
	"time"
	"ttnmapper-mysql-insert-agg/types"
)
...
var (
	dbInserts = promauto.NewCounter(prometheus.CounterOpts{
		Name: "ttnmapper_mysql_inserts_agg_count",
	...
	inserDuration = promauto.NewHistogram(prometheus.HistogramOpts{
		Name:    "ttnmapper_mysql_inserts_agg_duration",
...
func insertToMysql() {
	...
	stmtIns, err := db.PrepareNamed("INSERT INTO packets " +
		"(time, gtw_id, modulation, " +
		"datarate, bitrate, coding_rate, snr, rssi, " +
		"frequency, latitude, longitude, altitude) " +
		"VALUES " +
		"(:time, :gtw_id, :modulation, " +
		":datarate, :bitrate, :coding_rate, :snr, :rssi, " +
		":frequency, :latitude, :longitude, :altitude)")
...
func messageToEntry(message types.TtnMapperUplinkMessage, gateway types.GatewayMetadata) types.MysqlAggPacket {
	var entry = types.MysqlAggPacket{}
